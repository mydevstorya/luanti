	...
	if #external_mods == 0 then
		...
		if selected_external_mod then
			...
			if selected_external_mod.is_modpack then
				is_enabled = pkgmgr.is_modpack_entirely_enabled(
					data.list:get_raw_list(), selected_external_mod)
			end
...
local function handle_buttons(this, fields)
	...
	if fields.btn_toggle_mod then
		...
		if filtered_to_original and external_mods then
			...
			if mod then
				if mod.is_modpack then
					local entirely_enabled = pkgmgr.is_modpack_entirely_enabled(
						this.data.list:get_raw_list(), mod)
